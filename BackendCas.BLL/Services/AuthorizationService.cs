...
public class AuthorizationService : IAutorizacionService
{
    ...
    public async Task<AuthorizationResponse> ObtainToken(AuthorizationRequest authorization)
    {
        ...
        if (usuario_encontrado == null) return await Task.FromResult<AuthorizationResponse>(null);


        var tokenCreado = GenerateToken(usuario_encontrado.IdAdministrator.ToString());
