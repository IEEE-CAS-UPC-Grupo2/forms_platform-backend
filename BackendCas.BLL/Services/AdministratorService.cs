...
namespace BackendCas.BLL.Services
{
    public class AdministratorService:IAdministratorService
    {
        ...
        async Task<AdministratorDTO> IAdministratorService.Create(BackendCas.DTO.AdministratorDTO modelo)
        {
            try
            {
        ...
        async Task<bool> IAdministratorService.Edit(BackendCas.DTO.AdministratorDTO modelo)
        {
            try
            {
                var productoModelo = _mapper.Map<Administrator>(modelo);

                var productoEncontraro = await _PruebaRepository.Obtain(u => u.IdAdministrator==productoModelo.IdAdministrator);
                if (productoEncontraro==null)
                {
                    throw new TaskCanceledException("The administrator doesn't exist");
                ...
                bool answer = await _PruebaRepository.Edit(productoEncontraro);

                if (!answer)
                {
                    throw new TaskCanceledException("The administrator doesn't edit");
        ...
        async Task<bool> IAdministratorService.Delete(int id)
        {
            try
            {
                ...
                bool answer = await _PruebaRepository.Delete(productoEncontrado);
                if (!answer)
                {
                    throw new TaskCanceledException("The administrator doesn't delete");
